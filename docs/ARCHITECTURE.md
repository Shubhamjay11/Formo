...
| Auth | Better Auth | Email+password, sessions; Google OAuth fast-follow |
...
## Multi-tenancy
Organization = tenant (UI: "workspace"). users ↔ memberships(role) ↔ organizations.
...
Signup auto-creates a personal workspace (org + owner membership).
...
- Feature task lists and checkboxes live only in `docs/features/` specs — never here
